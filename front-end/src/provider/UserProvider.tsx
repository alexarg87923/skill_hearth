...
function useUserContextState(init?: User | null) {
    ...
    const [userContext, setUserContext] = useState((local_storage ? JSON.parse(local_storage) : null) || init);
	...
	useEffect(() => {
		...
		const verifyAndDecodeCookie = async () => {
			...
			try {
                ...
                const response = await apiAxios.get('/auth/verify-session', {signal});
